...
def run_patient_search(query: str) -> List[PatientMatch]:
    ...
    if ds == "db":
        from openemr_mcp.repositories.patient import search_patients, get_openemr_connection
...
def run_get_patient_by_id(pid: int) -> Optional[PatientMatch]:
    if pid <= 0:
        return None
    ds = get_effective_data_source()
    if ds == "db":
        from openemr_mcp.repositories.patient import get_patient_by_id, get_openemr_connection
