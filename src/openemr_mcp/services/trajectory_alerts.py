...
from openemr_mcp.schemas import MetricTrajectory, DriftAlert, TrajectoryPoint
...
def _baseline_value(points: List[TrajectoryPoint], baseline_months: int) -> Optional[float]:
...
def _check_a1c(traj: MetricTrajectory) -> Optional[DriftAlert]:
    if not traj.points:
        return None
    ...
    if delta >= 0.5:
        return DriftAlert(
            metric="a1c", severity="high", title="HbA1c Worsening",
...
def _check_ldl(traj: MetricTrajectory) -> Optional[DriftAlert]:
    if not traj.points:
        return None
    sorted_pts = sorted(traj.points, key=lambda p: p.effective_at)
    latest = sorted_pts[-1].value
    ...
    if delta >= 40:
        return DriftAlert(
            metric="ldl", severity="high", title="LDL Cholesterol Significantly Elevated",
    ...
    if delta >= 20:
        return DriftAlert(
            metric="ldl", severity="moderate", title="LDL Cholesterol Rising",
            rationale=f"LDL increased by {delta:.0f} mg/dL (from {baseline:.0f} to {latest:.0f} mg/dL) over 6 months.",
            evidence_points=_points_in_last_n_months(sorted_pts, 6),
        )
    return None
...
def _check_egfr(traj: MetricTrajectory) -> Optional[DriftAlert]:
    if not traj.points:
        return None
    sorted_pts = sorted(traj.points, key=lambda p: p.effective_at)
    latest = sorted_pts[-1].value
    ...
    if baseline is not None:
        ...
        if decline >= 5:
            decline_alert = True
            rationale_parts.append(f"eGFR declined by {decline:.1f} mL/min/1.73m² (from {baseline:.1f} to {latest:.1f}) over 6 months")
    if latest < 60:
    ...
    if decline_alert:
        return DriftAlert(
            metric="egfr", severity="high", title="eGFR Declining — Kidney Function Concern",
...
def _check_weight(traj: MetricTrajectory) -> Optional[DriftAlert]:
    if not traj.points:
        return None
    sorted_pts = sorted(traj.points, key=lambda p: p.effective_at)
    latest = sorted_pts[-1].value
    ...
    if pct_change >= 5:
        ...
        return DriftAlert(
            metric="weight", severity="moderate", title=f"Significant Weight Change ({direction.title()})",
...
def _check_bp(systolic_traj: Optional[MetricTrajectory], diastolic_traj: Optional[MetricTrajectory]) -> List[DriftAlert]:
    ...
    if avg_sys is not None and avg_sys >= 160:
        alerts.append(DriftAlert(metric="bp_systolic", severity="high", title="Severely Elevated Systolic Blood Pressure",
            rationale=f"Average systolic BP over last 3 readings is {avg_sys:.0f} mmHg (≥160 mmHg).", evidence_points=evidence))
    elif avg_sys is not None and avg_sys >= 140:
        alerts.append(DriftAlert(metric="bp_systolic", severity="moderate", title="Elevated Systolic Blood Pressure",
            rationale=f"Average systolic BP over last 3 readings is {avg_sys:.0f} mmHg (≥140 mmHg).", evidence_points=evidence))
    if avg_dia is not None and avg_dia >= 100:
        alerts.append(DriftAlert(metric="bp_diastolic", severity="high", title="Severely Elevated Diastolic Blood Pressure",
            rationale=f"Average diastolic BP over last 3 readings is {avg_dia:.0f} mmHg (≥100 mmHg).", evidence_points=evidence))
    elif avg_dia is not None and avg_dia >= 90:
        alerts.append(DriftAlert(metric="bp_diastolic", severity="moderate", title="Elevated Diastolic Blood Pressure",
            rationale=f"Average diastolic BP over last 3 readings is {avg_dia:.0f} mmHg (≥90 mmHg).", evidence_points=evidence))
    return alerts


def _check_phq9(traj: MetricTrajectory) -> Optional[DriftAlert]:
    if not traj.points:
        return None
    sorted_pts = sorted(traj.points, key=lambda p: p.effective_at)
    latest = sorted_pts[-1].value
    ...
    if latest_band == "severe" and baseline_band != "severe":
        return DriftAlert(
            metric="phq9", severity="high", title="PHQ-9 Score Entered Severe Range",
    ...
    if delta >= 5:
        return DriftAlert(
            metric="phq9", severity="moderate", title="PHQ-9 Score Worsening",
...
def compute_drift_alerts(trajectories: List[MetricTrajectory]) -> List[DriftAlert]:
    ...
    for metric, check_fn in [("a1c", _check_a1c), ("ldl", _check_ldl), ("egfr", _check_egfr), ("weight", _check_weight)]:
